...
if not binaryedge_api_key:
    binaryedge_api_key = "b0a47305-8f18-445b-827f-bd5945b6c583"  # Replace with your actual BinaryEdge API key
    ...
    while attempt < retries:
        try:
            ...
            if response.status_code == 401:
                logging.error("Unauthorized access: Please check your SIEM token and ensure it has the correct permissions.")
            elif response.status_code == 403:
                logging.error("Forbidden: You don't have permission to access this resource. Verify your SIEM API key permissions.")
            elif response.status_code == 404:
                logging.error("Not Found: The SIEM API URL is incorrect. Please check the URL.")
            elif response.status_code >= 500:
                logging.error("Server error: The SIEM server is currently unavailable. Retry later.")
...
if __name__ == "__main__":
    real_time_integration_process(
        ...
        binaryedge_api_key = os.getenv('BINARYEDGE_API_KEY', 'b0a47305-8f18-445b-827f-bd5945b6c583'),
        config_file = r"schema_mapping.yaml",
        db_connection_string=os.getenv('DB_CONNECTION_STRING', 'sqlite:///enriched_alerts.db'),
